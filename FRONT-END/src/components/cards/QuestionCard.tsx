...
export default function TicketCard({ ticket, onVote, showActions = false }: TicketCardProps) {
          ...
          <div className="flex items-center space-x-2">
            <Avatar className="w-6 h-6">
              ...
              <AvatarFallback className="bg-primary-light text-primary text-xs">
                {ticket.author.name.split(' ').map(n => n[0]).join('')}
              </AvatarFallback>
            </Avatar>
        ...
        <div className="flex items-center space-x-3">
          ...
          <div className="flex items-center space-x-1">
            <Button
              variant="ghost"
              size="sm"
              className={`h-8 w-8 p-0 ${isUpvoted ? 'text-primary bg-primary-light' : 'text-muted-foreground hover:text-primary'}`}
              onClick={(e) => {
                e.preventDefault();
                handleVote('up');
              }}
            ...
            <Button
              variant="ghost"
              size="sm"
              className={`h-8 w-8 p-0 ${isDownvoted ? 'text-destructive bg-red-50' : 'text-muted-foreground hover:text-destructive'}`}
              onClick={(e) => {
                e.preventDefault();
                handleVote('down');
              }}
